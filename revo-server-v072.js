...
async function verifyRevoTransaction({ amount, txHash, from, receiver }) {
  ...
  try {
    // Connection proof: ask Libertas for the network chain ID first.
    ...
    if (txFrom !== String(from).trim().toLowerCase()) {
      return { ok: false, error: "Blockchain sender does not match the user's authorized REVO wallet." };
    }
    ...
    // REVO is the native currency on Libertas. For a native transfer,
    // tx.value is the amount sent in the smallest denomination.
